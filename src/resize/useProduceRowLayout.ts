...
export const useProduceRowLayout = (ref: TableElementRef, selector: string) => {
  ...
  React.useLayoutEffect(() => {
    const allCells = Array.from(ref.current!.querySelectorAll(selector));

    const stiffCells = Array.from(ref.current!.querySelectorAll(`${selector}.stiff`));
    ...
    const normalCells = Array.from(ref.current!.querySelectorAll(`${selector}:not(.stiff)`));

    tableMemoryRef.current!.resizedLayout = allCells.map((cell, index) => {
