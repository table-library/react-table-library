...
While navigating through the documentation, you will see both, `CompactTable`s and Composable Tables. Therefore you will need to know when to use which:

## Compact Table

The `CompactTable` is the straightforward yet basic usage of the table library: It's one table component to rule them all. All configuration of the table can be provided via props to this component. Under the hood, the `CompactTable` is an assembled composed table. Usually you will want to use `CompactTable` for any case. However, once it gets more complex, you may want to use the Composable Table ...
